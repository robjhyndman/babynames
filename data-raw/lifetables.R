...
get_lifetables <- function(year) {
  ...
  tab <- xml2::read_html(sprintf(url, year)) |> rvest::html_table(fill = TRUE)
  tab <- tab[[2]]
  ...
  dat[-8] <- lapply(dat[-8], function(x) {
    zapsmall(readr::parse_number(x))
  })
...
years <- seq(1900, 2100, by = 10)
years <- years[years <= as.numeric(format(Sys.Date(), "%Y"))]
...
usethis::use_data(lifetables, compress = "xz", overwrite = TRUE)
